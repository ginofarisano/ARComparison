...
public class MenuOptions : MonoBehaviour
{
	...
	private NoStaticAmbientExtendedTrackingManager noStaticAmbientExtendedTrackingManager;
    ...
    {	
		noStaticAmbientExtendedTrackingManager = FindObjectOfType<NoStaticAmbientExtendedTrackingManager> ();
		stereoToMono = FindObjectOfType<StereoToMono> ();
	...
	{
		if(noStaticAmbientExtendedTrackingManager==null)
			noStaticAmbientExtendedTrackingManager = FindObjectOfType<NoStaticAmbientExtendedTrackingManager> ();
...
#else
		UnityEngine.SceneManagement.SceneManager.LoadScene("AR-1-About");
#endif
    }
	...
	/// <summary>
	/// Toggles the no static ambient extended tracking manager. Added by Gino Farisano
	/// </summary>
	public void ToggleNoStaticAmbientExtendedTracking()
	{
		Toggle noStaticAmbientExtendedTrackingToggle = FindUISelectableWithText<Toggle>("ET no static ambient");

			if (noStaticAmbientExtendedTrackingToggle && noStaticAmbientExtendedTrackingManager) 
				noStaticAmbientExtendedTrackingManager.activate(noStaticAmbientExtendedTrackingToggle.isOn);
		
		CloseMenu();
	}
	/// <summary>
	/// Toggles the stereo to mono. Added by Gino Farisano
	/// </summary>
	public void ToggleStereoToMono()
	{
		Toggle stereoToMonoToggle = FindUISelectableWithText<Toggle>("Mono");
		
		if (stereoToMonoToggle && stereoToMono)
			stereoToMono.switchVisualization(stereoToMonoToggle.isOn);
    ...
    public void UpdateUI()
    {
		...
		Toggle noStaticAmbientExtendedTrackingToggle = FindUISelectableWithText<Toggle>("ET no static ambient");
		if (noStaticAmbientExtendedTrackingToggle && noStaticAmbientExtendedTrackingManager) 
			noStaticAmbientExtendedTrackingToggle.isOn = noStaticAmbientExtendedTrackingManager.isActive();

		Toggle stereoToMonoToggle = FindUISelectableWithText<Toggle>("Mono");
		if (stereoToMonoToggle && stereoToMono) 
			stereoToMonoToggle.isOn = stereoToMono.isMono();
